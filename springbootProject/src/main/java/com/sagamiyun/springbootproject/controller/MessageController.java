...
import com.sagamiyun.springbootproject.entity.Message;
import com.sagamiyun.springbootproject.entity.User;
import com.sagamiyun.springbootproject.mapper.MessageMapper;
...
public class MessageController extends BaseController {
    ...
    // 查询所有数据
    @GetMapping("/foreign/{foreign_id}")
    public Result<?> foreign(@PathVariable Integer foreign_id) {
        return Result.success(findByForeign(foreign_id));
    }
    ...
    private List<Message> findByForeign(Integer foreign_id) {
        // 根据 foreign_id 0 查询所有的留言数据
        LambdaQueryWrapper<Message> queryWrapper = Wrappers.<Message>lambdaQuery().eq(Message::getForeign_id, foreign_id).orderByDesc(Message::getId);
        ...
        for (Message Message : list) {
            ...
            Long parentId = Message.getParent_id();
